R = 20    # R, CIRCUMFERENCE : 5,24  8,40  20,108
...
shapes.circle(BRICKS, rpos(0, 0, 0), R, Axis.Y, ShapeOperation.HOLLOW)
...
'''
# base
#shapes.circle(BLOCK_OF_QUARTZ, rpos(0, 1, 0), R, Axis.Y, ShapeOperation.REPLACE)
#shapes.circle(BLOCK_OF_QUARTZ, rpos(0, 0, 0), R+1, Axis.Y, ShapeOperation.REPLACE)
...
# wall
arch_wall(outline, plan, H)
arch_wall(outline, plan, 2*H)

def arch_wall(outline : List[Position], plan : List[number], top : number):
    for g in range(CIRCUMFERENCE // len(plan)):
        for i in range(len(plan)):
            for y in range(plan[i]):
                blocks.place(BLOCK_OF_QUARTZ, positions.add(outline[g*len(plan)+i], pos(0, 1+top-y,0)))
