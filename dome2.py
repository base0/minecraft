...
player.teleport(rpos(44, 28, -45))

#loops.run_in_background(roof)
...
def wall():
    global outline1
    global outline2
    shapes.circle(BLOCK_OF_QUARTZ, rpos(0, 0, 0), R1, Axis.Y, ShapeOperation.REPLACE)
    arch_wall(outline1, plan, H)
    shapes.circle(BLOCK_OF_QUARTZ, rpos(0, H, 0), R1, Axis.Y, ShapeOperation.REPLACE)
    arch_wall(outline1, plan, 2*H)
    shapes.circle(BLOCK_OF_QUARTZ, rpos(0, 2*H, 0), R1, Axis.Y, ShapeOperation.REPLACE)
    arch_wall(outline2, plan, 3*H)
